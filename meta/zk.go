...
func ZkWatcher(zconn *zookeeper.Conn) error {
	var err error
	for {
		ccDirPath := "/r3/app/" + theMetadata.appName + "/controller"
		clusterLeader, regionLeader, watcher, err := ElectLeader(zconn, ccDirPath, theMetadata.localRegion, true)
		if err != nil {
			log.Println(err)
			break
		}
		fmt.Println("leader:", clusterLeader, regionLeader)

		if theMetadata.clusterLeaderZNodeName != clusterLeader {
			theMetadata.clusterLeaderZNodeName = clusterLeader
			// 获取ClusterLeader配置
		...
		if theMetadata.regionLeaderZNodeName != regionLeader {
			theMetadata.regionLeaderZNodeName = regionLeader
		...
		e := <-watcher
		log.Println("zleader", e)
	}
	log.Printf("zk: watcher quit, %v", err)
	return err
}
